...
setwd("/media/london_A/mengxin/GTOP_code/extend/extend_3/input")

# Extend.Data.Fig.3a ASE & ASTS ----------------------------------------------

df_plot <- fread("Exfig 3a.txt")
reg <- lm(formula = sig.ase ~ total.ase,data=df_plot)
coeff <- coefficients(reg)
intercept <- coeff[1]
slope <- coeff[2]
p1 <- ggplot(df_plot,aes(x=total.ase,y=sig.ase,color=Abbreviation)) + geom_point(size=3) + theme_pubr() + 
...
df_sample.l <- fread("Exfig 3b.txt") %>% 
